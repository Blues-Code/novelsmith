...
- `novelsmith status --chapters`：显示每章状态和 failed 章节的 critical issues
- 审计 JSON 解析容错（#51）
...
- `novelsmith up` 自动写入 `novelsmith.log`，守护进程重启后可追溯
...
- `novelsmith doctor` 失败时给出针对性 hints（检查 baseUrl、试 stream:false、检查 API Key）
...
把已有的小说（单文件或章节目录）导入 novelsmith，系统自动拆章、逆向工程生成全套真相文件（世界状态、伏笔、角色矩阵等），之后直接 `write next` 续写。

```bash
novelsmith import chapters 我的小说 --from 已有章节/        # 从目录导入
novelsmith import chapters 我的小说 --from 全书.txt          # 从单文件导入（自动按"第X章"拆分）
novelsmith import chapters 我的小说 --from 全书.txt --split "Chapter\\s+\\d+"  # 自定义分章正则
novelsmith write next 我的小说                               # 无缝续写
```
...
```bash
novelsmith import canon 烈焰前传 --from 吞天魔帝   # 导入正传正典到番外
novelsmith write next 烈焰前传                     # 写手自动读取正典约束
```
...
```bash
novelsmith style analyze 参考小说.txt                     # 分析：句长、TTR、修辞特征
novelsmith style import 参考小说.txt 吞天魔帝 --name 某作者  # 导入文风到书
```
...
```bash
novelsmith config set-model writer gpt-4o-mini                                    # 简单模型覆盖
novelsmith config set-model auditor gemini-2.5-flash \
  --base-url https://generativelanguage.googleapis.com/v1beta/openai \
  --provider openai \
  --api-key-env GEMINI_API_KEY                                                # 走 Gemini API
novelsmith config set-model reviser claude-sonnet-4-20250514 \
  --base-url https://api.anthropic.com \
  --provider anthropic \
  --api-key-env ANTHROPIC_API_KEY                                             # 走 Anthropic API
novelsmith config show-models                                                      # 查看路由全景
```
...
```bash
novelsmith analytics 吞天魔帝          # 审计通过率、高频问题类别、问题最多的章节
novelsmith analytics 吞天魔帝 --json   # 结构化输出
```
...
- `novelsmith export` 支持 `--format md`、`--output <path>`、`--approved-only`
...
| AIGC 检测 API | 外部 API 集成（GPTZero / Originality / 自定义端点），`novelsmith detect` 命令 |
| 文风指纹学习 | 从参考文本提取 StyleProfile（句长、TTR、修辞特征），注入 Writer prompt |
| 反检测改写 | ReviserAgent `anti-detect` 模式，检测→改写→重检测循环 |
| 检测反馈闭环 | `detection_history.json` 记录每次检测/改写结果，`novelsmith detect --stats` 查看统计 |

```bash
novelsmith style analyze reference.txt         # 分析参考文本文风
novelsmith style import reference.txt 吞天魔帝  # 导入文风到书
novelsmith detect 吞天魔帝 --all               # 全书 AIGC 检测
novelsmith detect --stats                      # 检测统计
```
...
```bash
novelsmith book create --title "吞天魔帝" --genre xuanhuan
```

题材规则可以查看、复制到项目中修改、或从零创建：

```bash
novelsmith genre list                      # 查看所有题材
novelsmith genre show xuanhuan             # 查看玄幻的完整规则
novelsmith genre copy xuanhuan             # 复制到项目中，随意改
novelsmith genre create wuxia --name 武侠   # 从零创建新题材
```
...
- `novelsmith update` 一键更新、`novelsmith init` 支持当前目录初始化
- API 错误附带中文诊断提示，`novelsmith doctor` 含 API 连通性测试
