...
import { listAvailableGenres, readGenreProfile, getBuiltinGenresDir } from "@mrweijh/novelsmith-core";
  ...
  .action(async (id: string) => {
    try {
      ...
      try {
        ...
        logError(`Built-in genre "${id}" not found. Use 'novelsmith genre list' to see available genres.`);
        process.exit(1);
        return;
      }
