...
configCommand
  .command("set-global")
  .description("Set global LLM config (~/.novelsmith/.env), shared by all projects")
  ...
  .action(async (opts) => {
    try {
      ...
      const lines = [
        "# novelsmith Global LLM Configuration",
        `NOVELSMITH_LLM_PROVIDER=${opts.provider}`,
        `NOVELSMITH_LLM_BASE_URL=${opts.baseUrl}`,
        `NOVELSMITH_LLM_API_KEY=${opts.apiKey}`,
        `NOVELSMITH_LLM_MODEL=${opts.model}`,
      ];
      if (opts.temperature) lines.push(`NOVELSMITH_LLM_TEMPERATURE=${opts.temperature}`);
      if (opts.maxTokens) lines.push(`NOVELSMITH_LLM_MAX_TOKENS=${opts.maxTokens}`);
      if (opts.thinkingBudget) lines.push(`NOVELSMITH_LLM_THINKING_BUDGET=${opts.thinkingBudget}`);
      if (opts.apiFormat) lines.push(`NOVELSMITH_LLM_API_FORMAT=${opts.apiFormat}`);
      if (opts.lang) lines.push(`NOVELSMITH_DEFAULT_LANGUAGE=${opts.lang}`);
...
configCommand
  .command("show-global")
  .description("Show global LLM config (~/.novelsmith/.env)")
  .action(async () => {
    try {
      const content = await readFile(GLOBAL_ENV_PATH, "utf-8");
      const masked = content.replace(
        /(NOVELSMITH_LLM_API_KEY=)(.{8})(.*)(.{4})/,
        "$1$2...$4",
      );
      log(masked);
    } catch {
      log("No global config found. Run 'novelsmith config set-global' to create one.");
    }
  });
