...
export const updateCommand = new Command("update")
  .description("Update novelsmith to the latest version")
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view @mrweijh/novelsmith version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g @mrweijh/novelsmith@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g @mrweijh/novelsmith@latest");
      process.exit(1);
    }
  });
