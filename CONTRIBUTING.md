...
```bash
git clone https://github.com/Narcooo/novelsmith.git
cd novelsmith
pnpm install
pnpm build
pnpm test
...
```bash
pnpm --filter @mrweijh/novelsmith-core test    # Core tests only
pnpm --filter @mrweijh/novelsmith test         # CLI tests only
```
...
Open an issue or check existing ones: https://github.com/Narcooo/novelsmith/issues
